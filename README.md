...
Planned Features:
- Silent intall via CMD. Creating a CMD syntax to use this tool without the GUI, that way the driver can be loaded without display.
- A UI Theme/Overhaul.
- Im gonna make it so it can be ran from a staging tool/sunshine or any process that can load it directly or a .bat that calls it. Also likely I will create a generic installer flow with options for users who dont need the full GUI and just wanna get up and going
- the plan for the final status of the tool it will be setup to push settings to it via CMDs so in theory you can use a staging tool, sunshine run command feature, or anything that can run a command or execute a .bat will be able to load initial settings or toggle install/uninstall/reload. This way you could load different clients with the driver configured completely differently. This would be useful for batch setup of a Gaming server that has multiple VMs that need sunshine installed and these drivers configured, Ive seen people do that.
