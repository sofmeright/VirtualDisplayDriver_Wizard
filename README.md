...
- Spoke too soon, I will fix this next: Solution - In order to have the selected RenderEngine save to file, you will need to edit a priority. (You can even press F2 then Press ENTER) There is no logic to detect when a user makes a selection in the combo box and in turn to write to file. For now its recommended to use this temporary workaround.
