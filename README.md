...
- Silent intall via CMD. Creating a CMD syntax to use this tool without the GUI, that way the driver can be configured from other tools. In essence allow operation from a staging tool/sunshine or any process that can load it directly or a .bat that calls it. This will allow users to load initial settings or toggle install/uninstall/reload. Users could also bulk configure many different gaming servers with the driver configured completely differently in an efficient manner by this feature. This would be useful for Gaming servers that have multiple VMs that need sunshine installed and need for these drivers configured.
