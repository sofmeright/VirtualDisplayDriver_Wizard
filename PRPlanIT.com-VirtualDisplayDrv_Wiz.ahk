...
Gui, Add, ListView, -ReadOnly NoSort R21 AltSubmit vListView_Resolutions gResolutionWasChanged, Priority|Width|Height|Refresh Rate
...
if A_Args.Length() = 0
	...
	else if (A_Args.Length() = 1) or (A_Args.Length() = 2) or (A_Args.Length() = 4)
		{
			; "Driv_Inst/Unin/Relo"
			; "Back_Load/Save/Remo" <filename>
			; "Moni_Sets" <integer>
			; "GPUs_Sets" <String> / or <integer>
			; "Reso_Adds/Remo" %SUNSHINE_CLIENT_WIDTH% %HEIGHT% %FPS% ; Add has to set to the highest priority.
			if (Exec_Inte = "Reso_Adds") or (Exec_Inte = "Reso_Remo")
				{
					if !RegExMatch(Exec_Args, "[0-9]+ [0-9]+ [0-9]+")
						Exec_Inte := ""
				}
			; MsgBox % "A command has been executed."
			Gui, Show, Hide, VirtualDisplayDrvWiz
		}
		else 
			{
				ExitApp
				...
				Else If Exec_Inte = Back_Load
					{
						...
						ControlGet, BackupFileLst, List,, ComboBox1, VirtualDisplayDrvWiz
						BackupDetected := 0
...
Driver_GetInfName()
	{
		...
		RegExMatch(CMDtoSTDOut("pnputil /enum-devices /deviceid ROOT\iddsampledriver"), "Driver Name:[ ]+(oem[0-9]+.inf)", InfName)
		return InfName1
	}
DriverInstall()
{
